...
{ 
	...
	while (!QueueIsEmpty (Fila)){
        	...
        	sum += Node->Elem;
       		...
       		if (Node->PtLeft != NULL) 
			QueueEnqueue (Fila, &Node->PtLeft);

        	if (Node->PtRight != NULL) 
			QueueEnqueue (Fila, &Node->PtRight);
    	}
...
{ 
	...
	if(proot->PtLeft != NULL)
		odd += ABPOddCount(proot->PtLeft);

	if(proot->PtRight != NULL)
		odd += ABPOddCount(proot->PtRight);
...
{ 
	...
	if(proot->PtLeft != NULL)
		sum += ABPEvenSum(proot->PtLeft);

	if(proot->PtRight != NULL)
		sum += ABPEvenSum(proot->PtRight);
